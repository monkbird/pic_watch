...
## 版本更新
- v1.0.7（2025-12-02）
  - 核心修复：引入 piexifjs，实现真实的 JPEG 元数据（备注/标记）写入，修复了之前“假写入”的问题。
  - 照片卡片文字改为 黑色粗体（SimHei），增强可读性。
  - 修复了卡片尺寸显示错误的 Bug。
  - 移除了网格间距，实现完全紧凑的图片排列。
  - 大图查看支持 Ctrl+滚轮缩放 及 左键拖拽。
  - 自动记忆上次导入的文件夹路径，下次打开自动加载。
  - 详情面板和大图顶部栏增加显示文件的 真实完整路径。
- v1.0.6（2025-11-28）
